...
export default {
  ...
  mounted () {
    ...
    // 李钊鸿加入的代码
    let lastIndex = arrayFindIndex(this.$children, item => item.id === this.currentActive);
    this.wrap.style.webkitTransform = `translate3d(${-lastIndex * this.pageWidth}px, 0, 0)`;
  },
  ...
  methods: {
    ...
    swipeMove(offset) {
        // 李钊鸿加入的代码：
        this.$store.dispatch('set_translateX',`${offset}px`)
