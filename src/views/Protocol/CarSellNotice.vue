    ...
    <div class="CarSellNotice">
        <div class="Notice">
                ...
                <div>
                    ...
                    <p class="Notice--indent">一、 竞买标的： 车牌号： {{ d.licensePlateNumber }}； 车辆型号：{{ d.carModel }}； 车辆识别代号： {{ d.frameNumber }}； 发动机号： {{ d.engineNumber  }}； 初次登记日期： {{ d.registerDate   }}； 行驶总里程： {{ d.mileage }}KM。</p>
                    ...
                    <p class="Notice--indent"> <span class="Notice--bold">六、 本标的物竞得者应将竞买款在 {{ d.paymentEndTime }} 前缴入我方指定账户（户名：{{ d.account }}  开户银行：{{ d.bank }}  账号： {{ d.cardNo }}）。</span></p>
